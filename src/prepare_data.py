...
def create_capitalization_label(word: str):
    if word[0].isupper():
        return "U"
    return "O"


def create_punctuation_label(word: str, punct_marks: str):
    return word[-1] if word[-1] in punct_marks else "O"


def create_text_and_labels(
    output_dir: str, file_path: str, use_punctuation: bool = False
):
            ...
            with open(labels_file, "w", encoding="utf-8") as labels_f:
                for line in f:
                    ...
                    for word in line:
                        ...
                        # punctuation
                        if use_punctuation:
                            label = create_punctuation_label(word, punct_marks=CHARS)
                        word = remove_punctuation(word)

                        # capitalization
                        if len(word) > 0:
                            label += create_capitalization_label(word)
                            word = word.lower()
                            text += word + " "
                            labels += label + " "
